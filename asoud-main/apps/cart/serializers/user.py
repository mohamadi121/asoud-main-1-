...
class OrderItem2Serializer(serializers.ModelSerializer):
    ...
    def save(self, **kwargs):
        """Custom save method that properly handles order assignment"""
        order = kwargs.pop('order', None)
        if not order:
            raise ValueError("Order is required for creating OrderItem")
        ...
        affiliate = self.validated_data.get('affiliate', None)
        affiliate_name = self.validated_data.get('affiliate_name', None)
        quantity = self.validated_data.get('quantity', 1)
        
        # Validate that at least one product/affiliate is provided
        if not any([product, product_name, affiliate, affiliate_name]):
            raise ValueError("Either product, product_name, affiliate, or affiliate_name must be provided")
        
        # Create OrderItem based on what was provided
        order_item = None
        ...
        try:
            if product:
                ...
                try:
                    ...
                    raise ValueError(f"Affiliate product with name '{affiliate_name}' not found")
