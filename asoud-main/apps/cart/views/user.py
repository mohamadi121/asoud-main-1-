...
class CartViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        """Get cart contents - shows all items in the current order"""
        try:
        ...
        try:
            ...
            if product:
                ...
                if existing_item:
                    ...
                    existing_item.save()
                    response_serializer = OrderItem2Serializer(existing_item)
                    return Response(response_serializer.data, status=status.HTTP_201_CREATED)
                else:
                    # Create new item
                    new_item = serializer.save(order=order)
                    response_serializer = OrderItem2Serializer(new_item)
                    return Response(response_serializer.data, status=status.HTTP_201_CREATED)
            
            elif product_name:
                # Check if product with name already exists in cart
                existing_item = order.items.filter(product__name=product_name).first()
                if existing_item:
                    existing_item.quantity += quantity
                    existing_item.save()
                    response_serializer = OrderItem2Serializer(existing_item)
                ...
                # Check if affiliate already exists in cart
                existing_item = order.items.filter(affiliate=affiliate).first()
                if existing_item:
                    existing_item.quantity += quantity
                    existing_item.save()
                    response_serializer = OrderItem2Serializer(existing_item)
                ...
                # Check if affiliate with name already exists in cart
                existing_item = order.items.filter(affiliate__name=affiliate_name).first()
                if existing_item:
                    existing_item.quantity += quantity
                    existing_item.save()
                    response_serializer = OrderItem2Serializer(existing_item)
                ...
                return Response(
                    {"error": "Either product_id, product_name, affiliate_id, or affiliate_name must be provided"},
