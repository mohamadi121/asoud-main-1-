...
from utils.validators import validate_mobile_number, validate_pin_code, sanitize_input
import logging
...
class PinCreateAPIView(views.APIView):
    ...
    def post(self, request, format=None):
        ...
        try:
            ...
            user_obj.set_pin(pin)  # Use secure PIN hashing
            user_obj.pin_expiry = timezone.now() + timedelta(minutes=2)
            user_obj.save()
            
            # Security: Log PIN generation without sensitive data
            if settings.DEBUG:
                logger.debug(f"PIN generated for user ending in {mobile_number[-4:]}")
            result = SMSCoreHandler.send_verification_code(mobile_number, pin)
            # Security: Log SMS sending result securely
            logger.info(f"SMS verification sent to user {mobile_number[-4:]:0>4}, result: {'success' if result else 'failed'}")
            # Security: Don't return PIN in production
            data = {"message": "PIN sent successfully"} if not settings.DEBUG else {"pin": pin}
...
class PinVerifyAPIView(views.APIView):
    ...
    def post(self, request, format=None):
        try:
            # Validate and sanitize input
            mobile_number = sanitize_input(request.data.get("mobile_number", ''))
            pin = sanitize_input(request.data.get("pin", ''))
            
            validate_mobile_number(mobile_number)
            validate_pin_code(pin)
            
        except ValidationError as e:
            logger.warning(f"Invalid input in PIN verification: {e}")
            response = ApiResponse(
                success=False,
                code=400,
                error={
